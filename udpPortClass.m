classdef udpPortClass < handle
  ...
  methods
    function self = udpPortClass()    # Constructor
      ...
      self.udp_01 = udpport ("LocalHost","192.168.1.4","LocalPort",8266)
    endfunction

    function clearPort(self)
      posLF = 0;
      do
        bytesAvailable = self.udp_01.NumBytesAvailable;
        if (bytesAvailable > 0)
          inUDPPort = char(read(self.udp_01,bytesAvailable))
    ...
    function [bytesAvailable,inChar] = readPort(self)
      ...
      until (self.udp_01.NumBytesAvailable == 0)
